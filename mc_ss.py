...
def print_class(fc, indent="  "):
    ...
    if fc['ifaces']:
        ifaces_clean = [i.replace('/','.')for i in fc['ifaces'] if 'minecraft' not in i and 'java' not in i]
        if ifaces_clean:
            print(f"{indent}{red('│')} {gray('implements')} {DG}{', '.join(ifaces_clean)}{RE}")
    ...
    SUS_KW = [
        'killaura','kill aura','triggerbot','trigger bot','aimbot','aim bot',
        'silentaim','forcefield','multiaura','bhop','bunnyhop','nofall','noclip',
        'antiknockback','antikb','velocityhack','flyhack','speedhack','scaffold',
        'wallhack','xrayhack','xray','playeresp','entityesp','cheststealer',
        'chamsmod','hitboxesp','hitbox esp','packetfly','reachmod','pingspoof',
        'nukermod','autofarm','autofish','autosteal','autoclick','critspam',
        'aimassist','aim assist','anti-aim','antiaim',
        'novaclient','nova client','wurst','liquidbounce','meteorclient',
        'impactclient','aristois','futureclient','sigmaclient','novoline',
        'vapeclient','wolframclient','backdoor','keylogger','tokenstealer',
        'discordstealer','webhook','authorized','unauthorized','stealer',
        'bypass','cheat','hack module','esp module','aura module',
    ]
    sus_strs = [s for s in strs if any(k in s.lower() for k in SUS_KW)]
    if sus_strs:
        print(f"{indent}{red('│')} {bold('Strings:')}")
        for s in sus_strs:
            print(f"{indent}{red('│')}   {red('▶')} {Y}{s}{RE}")
...
def scan_folder(folder):
    ...
    if dirty:
        ...
        print(f"\n  {bold('VERDICT')}  \033[42m\033[30m{bold('  ✓  ALL CLEAN  ')}{RE}")
    print()
    ...
    if os.path.isdir(inp):
        ...
        if m=='2': list_classes(inp)
            ...
            if not r: print(red("  Cannot read JAR."))
                ...
                print(f"\n  {tag_bad()} {red(str(len(r['flagged']))+' class(es) flagged')}\n")
                for fc in r['flagged']: print_class(fc)
    print()
