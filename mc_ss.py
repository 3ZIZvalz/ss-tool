...
def print_class(fc, indent="  "):
    ...
    if strs:
        ...
        for s in strs:
            # Highlight if it looks like a cheat keyword
            is_sus = any(k in s.lower() for k in [
                'kill','aura','trigger','aim','esp','fly','hack','speed',
                'scaffold','noclip','nofall','reach','velocity','nuker',
                'client','cheat','bypass','stealer','backdoor','webhook',
                'authorized','unauthorized','discord','token','password',
                'novaclient','wurst','meteor','liquid','sigma','vape',
                'autoclick','crit','force','multi','phase','wall','xray',
                'chest','steal','farm','fish','place','mine',
            ])
            if is_sus:
                print(f"{indent}{red('│')}   {red('▶')} {Y}{s}{RE}")
            else:
                print(f"{indent}{red('│')}   {gray('·')} {DG}{s}{RE}")
